...
// ── GET /lp/unsubscribe/:slug/:subscriberId — public, no auth ──
// Email Unsubscribe Phase A (2026-08-12). Recipient-specific, signed link
// generated by emailService.sendCampaignEmail. Deliberately public (no
// requireAuth) — the recipient of a campaign email has no QRAIVY account
// to log into. Tamper-resistance comes entirely from the HMAC signature
// (see unsubscribeToken.js), not from authentication: the signature binds
// subscriberId + slug + a dedicated purpose string, so a tampered id,
// tampered slug, or forged token is rejected before anything is read or
// written. Only affects EMAIL marketing (Subscriber.status/gdprConsent) —
// never touches LoyaltyCustomer, Customer, CustomerChannel, Wallet passes,
// WebPushSubscription, stamps, rewards, or deals.
async function handleUnsubscribe(req, res) {
  const { slug, subscriberId } = req.params;
  const token = req.query.t;
  const lang = req.query.lang || 'de';

  function renderPage(bizName, ok) {
    const isDE = lang === 'de';
    const title = isDE ? 'Abmeldung' : 'Unsubscribe';
    const heading = ok
      ? (isDE ? 'Erfolgreich abgemeldet' : "You're unsubscribed")
      : (isDE ? 'Ungültiger Link' : 'Invalid link');
    const body = ok
      ? (isDE
          ? `Du hast dich erfolgreich von den Marketing-E-Mails von ${bizName} abgemeldet.`
          : `You've been unsubscribed from email marketing for ${bizName}.`)
      : (isDE
...
<body style="margin:0;padding:0;background:#f4f4f4;font-family:Arial,sans-serif;display:flex;align-items:center;justify-content:center;min-height:100vh;">
  ...
  try {
    // Missing params, bad signature, and unknown subscriber all return the
    // identical generic response below — no information disclosure about
    // which specific check failed, and no database read/write happens
    // before the signature has already been verified.
    if (!slug || !subscriberId || !token || !verifyUnsubscribeToken(subscriberId, slug, token)) {
      return res.status(400).send(renderPage('', false));
    }
    ...
    if (subscriber.status !== 'unsubscribed') {
      ...
      // affect the result below. QRAIVY's own database is the source of
      // truth (per the approved Phase A design): a Resend API failure
      // must not restore marketing eligibility here, and must never
      // surface to the recipient — only logged server-side.
      if (process.env.RESEND_API_KEY && process.env.RESEND_AUDIENCE_ID && subscriber.email) {
        const { Resend } = require('resend');
        const client = new Resend(process.env.RESEND_API_KEY);
        client.contacts.update({ audienceId: process.env.RESEND_AUDIENCE_ID, email: subscriber.email, unsubscribed: true })
          .catch(e => console.error('[Unsubscribe] Resend contact sync failed (non-fatal):', e.message));
      }
    }
    // Already-unsubscribed: idempotent no-op — falls through to the same
    // success response below, no duplicate writes, no error.

    return res.status(200).send(renderPage(bizName, true));
  } catch(e) {
...
async function handleSendPush(req, res) {
  try {
    ...
    // gdprConsent AND status === 'subscribed'. Checking gdprConsent alone
    // (the prior behavior) would still include a subscriber who had
    // unsubscribed but whose gdprConsent flag was never separately reset —
    // status is the field the unsubscribe flow actually updates, so it
    // must be part of the eligibility filter, not just an informational
    // column nothing reads.
    const emailSubs = await prisma.subscriber.findMany({ where: { slug, gdprConsent: true, status: 'subscribed' }, select: { id: true, email: true } });
...
async function handleLoyaltyCardPage(req, res) {
  try {
    ...
    // Identity Continuity: this card must read the SAME customer-specific
    // Pass the Apple/Google Wallet buttons on this page enroll into — never
    // the legacy shared "sqr-{slug}" bucket (see utils/customerSerial.js).
    // Read-only lookup only: viewing this page must never create/upsert a
    // Pass row. When no valid cid is in the URL yet, cidBootstrapScript
    // below (mirroring handleServeLP's welcome-page redirect) resolves one
    // via the same getOrCreateCustomerId() already used for the wallet
    // buttons and redirects to this same URL with ?cid= attached, before
    // the stamp-count markup here is ever painted.
    const cid = (req.query && req.query.cid) ? String(req.query.cid).slice(0, 64) : null;
    const serial = getCustomerSerialNumber(slug, cid);
    const pass = serial ? await prisma.pass.findUnique({ where: { serialNumber: serial } }) : null;
