...
(function(){
  ...
  // Show notify button if not yet subscribed
  if('Notification' in window && Notification.permission !== 'denied') {
    var nb = document.createElement('div');
    nb.id = 'webpush-bar';
    nb.style.cssText = 'position:fixed;bottom:20px;left:50%;transform:translateX(-50%);background:#ff6b00;color:#fff;padding:12px 24px;border-radius:50px;font-size:.8rem;font-weight:700;cursor:pointer;z-index:9999;box-shadow:0 4px 20px rgba(0,0,0,0.3);white-space:nowrap;';
    nb.textContent = '🔔 Get notified of new deals';
    nb.onclick = function() {
      nb.textContent = '...';
      Notification.requestPermission().then(function(perm) {
        if(perm === 'granted' && window.__swReg) {
          fetch('https://api.qraivy.com/lp/webpush/vapid-key/' + slug)
            ...
            .then(function(){ nb.textContent = '✅ Notifications on!'; setTimeout(function(){ nb.remove(); }, 3000); })
            .catch(function(){ nb.remove(); });
        } else { nb.remove(); }
      });
    };
...
(function(){if('serviceWorker'in navigator&&'PushManager'in window){navigator.serviceWorker.register('/sw.js').then(function(reg){window.__swReg=reg;});}if('Notification'in window&&Notification.permission!=='denied'){var _slug=window.location.pathname.split('/').pop();var nb=document.createElement('div');nb.id='webpush-bar';nb.style.cssText='position:fixed;bottom:20px;left:50%;transform:translateX(-50%);background:#ff6b00;color:#fff;padding:12px 24px;border-radius:50px;font-size:.8rem;font-weight:700;cursor:pointer;z-index:9999;box-shadow:0 4px 20px rgba(0,0,0,0.3);white-space:nowrap;';nb.textContent='🔔 Get notified of new deals';nb.onclick=function(){nb.textContent='...';Notification.requestPermission().then(function(perm){if(perm==='granted'){var sub=function(reg){fetch('https://api.qraivy.com/lp/webpush/vapid-key/'+_slug).then(function(r){return r.json();}).then(function(d){var arr=new Uint8Array(atob(d.publicKey.replace(/-/g,'+').replace(/_/g,'/')).split('').map(function(c){return c.charCodeAt(0);}));return reg.pushManager.subscribe({userVisibleOnly:true,applicationServerKey:arr});}).then(function(s){var j=s.toJSON();return fetch('https://api.qraivy.com/lp/webpush/subscribe/'+_slug,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({endpoint:j.endpoint,keys:j.keys})});}).then(function(){nb.textContent='✅ Notifications on!';setTimeout(function(){nb.remove();},3000);}).catch(function(){nb.remove();});};if(window.__swReg){sub(window.__swReg);}else{navigator.serviceWorker.register('/sw.js').then(sub);}}else{nb.remove();}});};document.body.appendChild(nb);}})();
