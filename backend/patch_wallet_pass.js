...
// ── 1. Rewrite passService.js ─────────────────────────────────
const passServicePath = path.join(__dirname, 'src/services/passService.js');
const passService = `const JSZip = require('jszip');
...
fs.writeFileSync(passServicePath, passService, 'utf8');
console.log('1. passService.js rewritten');

// ── 2. Add wallet pass endpoint to lpController.js ────────────
const lpCtrlPath = path.join(__dirname, 'src/controllers/lpController.js');
let lp = fs.readFileSync(lpCtrlPath, 'utf8').replace(/\r\n/g, '\n');
...
const exportMarker = '\nmodule.exports';
if (lp.includes(exportMarker)) {
  lp = lp.replace(exportMarker, handlerCode + exportMarker);
  console.log('2. handleGenerateAppleWalletPass added to lpController');
} else {
  console.log('2. FAIL: module.exports not found in lpController');
}
...
lp = lp.replace(
  ...
  (match, inner) => {
    if (inner.includes('handleGenerateAppleWalletPass')) return match;
    return match.replace(inner, inner.trimEnd() + ',\n  handleGenerateAppleWalletPass,\n');
  }
);
console.log('3. handleGenerateAppleWalletPass exported');

// Update Apple Wallet button from "coming soon" to real link
const OLD_BTN = "sl.appleEnabled!==false?'<button class=\"lp-wallet-btn\">&#9679; Add to Apple Wallet &mdash; coming soon</button>':''";
const NEW_BTN = "sl.appleEnabled!==false?`<a href=\"/lp/wallet/apple/\${slug}\" class=\"lp-wallet-btn\" style=\"text-decoration:none;display:block;\">&#9679; Add to Apple Wallet</a>`:('')" ;
if (lp.includes(OLD_BTN)) {
  lp = lp.replace(OLD_BTN, NEW_BTN);
  console.log('4. LP Apple Wallet button updated to real link');
} else {
  console.log('4. WARN: Apple button string not found — checking partial match');
  if (lp.includes('coming soon')) console.log('   Found "coming soon" — manual fix may be needed');
}

fs.writeFileSync(lpCtrlPath, lp, 'utf8');

// ── 3. Add wallet route to lpRoutes.js ───────────────────────
const lpRoutesPath = path.join(__dirname, 'src/routes/lpRoutes.js');
let lr = fs.readFileSync(lpRoutesPath, 'utf8').replace(/\r\n/g, '\n');

// Add import of handleGenerateAppleWalletPass
if (!lr.includes('handleGenerateAppleWalletPass')) {
  lr = lr.replace(
    /const \{([^}]+)\}\s*=\s*require\(['"]\.\.\/controllers\/lpController['"]\)/,
    (match, inner) => match.replace(inner, inner.trimEnd() + ',\n  handleGenerateAppleWalletPass')
  );
  console.log('5. handleGenerateAppleWalletPass imported in lpRoutes');
} else {
  console.log('5. already imported');
}

// Add route BEFORE /:slug catch-all
const slugRoute = "router.get('/:slug'";
if (!lr.includes('/wallet/apple/') && lr.includes(slugRoute)) {
  lr = lr.replace(
    slugRoute,
    "router.get('/wallet/apple/:slug', handleGenerateAppleWalletPass);\n" + slugRoute
  );
  console.log('6. /wallet/apple/:slug route added');
} else {
  console.log('6. route already exists or /:slug not found');
}
